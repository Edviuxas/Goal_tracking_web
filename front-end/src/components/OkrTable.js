...
export default function OkrTable({ okrGoalsList, setOkrGoalsList }) {
  ...
  const handleClick = (event, id) => {
    ...
    } else if (selectedIndex > 0) { // if we have a row in the middle and it is already selected, we deselect it
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - okrGoalsList.length) : 0;

  const visibleRows = React.useMemo(
    () =>
      stableSort(okrGoalsList, getComparator(order, orderBy)).slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage,
      ),
    [order, orderBy, page, rowsPerPage, okrGoalsList],
  );
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
            ...
            okrGoalsList={okrGoalsList}
            setOkrGoalsList={setOkrGoalsList}
            selected={selected}
